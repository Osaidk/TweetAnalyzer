...
public class Filter {
    ...
    public static List<Tweets> inTimespan(List<Tweets> tweets, Timespan timespan) {
        ...
        for (Tweets tweet : tweets) {
            ...
            try {
                ...
                if (timespan.getStart().isBefore(time) && timespan.getEnd().isAfter(time)) {
                    filteredList.add(tweet);
                } } catch (Exception e1) {
                System.out.println("The list of tweets contains dirty data!");
            }
        }
        return filteredList;
    }
    ...
    public static List<Tweets> containing(List<Tweets> tweets, List<String> words) {
        List<Tweets> filteredList = new ArrayList<>();
